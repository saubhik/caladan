...
class UdpConn : public NetConn {
 public:
	...
	ssize_t ReadFrom(void *buf, size_t len, bool *isDecrypted, netaddr *raddr)
	{
		return udp_read_from(c_, buf, len, isDecrypted, raddr);
	}
	...
	{ return udp_read(c_, buf, len, nullptr); }
...
class TcpConn : public NetConn {
 ...
 public:
	...
	netaddr LocalAddr() const
	{ return tcp_local_addr(c_); }

	// Gets the remote TCP address.
	netaddr RemoteAddr() const
	{ return tcp_remote_addr(c_); }
